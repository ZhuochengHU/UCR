 ...
 *
 *	Demo: https://drive.google.com/file/d/1Y3LO51Fh7ndqG0nyGUlxvnOhpELMi7Aa/view?usp=sharing
 *
...
enum States_Speaker {start_speaker, init_speaker, power_speaker, up_speaker, down_speaker, standBy_speaker} state_speaker;
unsigned char threeLEDs = 0x00;
unsigned char blinkingLED = 0x00;
unsigned char speaker = 0x00;
unsigned int count_speaker = 0;
unsigned int count_melody = 1;
...
void tick_Speaker() {
	switch (state_speaker) {
		...
		case init_speaker:
			if((~PINA & 0x07) == 0x01) {
				state_speaker = up_speaker;
			} else if((~PINA & 0x07) == 0x02) {
				state_speaker = down_speaker;
	...
	switch (state_speaker) {
		...
		case init_speaker:
			...
			break;
		case power_speaker:
			if (count_speaker < count_melody) {
				speaker = 0x01;
			} else if (count_speaker >= count_melody) {
					if (count_speaker <= (2*count_melody)) {
						speaker = 0x00;
					} else {
						count_speaker = 0;
					}
